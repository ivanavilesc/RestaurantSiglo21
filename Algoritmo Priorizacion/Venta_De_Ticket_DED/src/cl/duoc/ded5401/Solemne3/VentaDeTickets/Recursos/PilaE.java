...
public class PilaE {
     ...
     */
    public void push(Ticket objTicket) {

        if (this.isEmpty()) {
            this.cabecera = new Nodo(objTicket);
        } else {
            this.getCima().setSiguiente(new Nodo(objTicket));
        }
        this.tam++;
    }
    ...
    public double calculaPesoPila(){
        ...
        if (!this.isEmpty()) {
            ...
            while (cont < this.tam) {
                sumaValor+=temp.getTicket().getNroTicket();
                temp = temp.getSiguiente();
                cont++;
            }
        } else {
    ...
    public boolean comparaPilas(PilaE pila1, PilaE pila2){
        ...
        if(pila1.tam==pila2.tam){
            ...
            for(int i=0; i<this.tam;i++){
                if(tempP1.getTicket().getNroTicket()== tempP2.getTicket().getNroTicket()){
                    valida = true;
